...
#    commands.getstatusoutput('rm -r robbed_bank_account')
    ...
    # now we use content_json dict to modify data
    commit, n_updates = process_repo('update', content_json)
    return Response("Elements update successfully; num_updates: {}, commit_sha: {}".format(n_updates, commit), status = 200)
...
def remove_elem():
    ...
    return Response("Elements deleted successfully; num_updates: {}".format(n_updates, commit), status = 200)
...
def process_repo(op, content_json):
    ...
    run_command("rm -r '{}'".format(TEMP_DIR, REPO_NAME))
    run_command("cd '{}'; git clone '{}'".format(TEMP_DIR, REPO_URL))
    # create new branch
    cmd_pref = "cd '" + TEMP_DIR + REPO_NAME + "'; "
    ...
    for path,dirs,files in os.walk(repo_dir):
        ...
        for fn in files:
            ...
            if fname.endswith('.yaml') or fname.endswith('.yml'):
                cc = process_file(fname, op, content_json)
                total_changes += cc
